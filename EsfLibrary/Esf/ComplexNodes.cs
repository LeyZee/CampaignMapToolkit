...
namespace EsfLibrary {
    ...
    public abstract class ParentNode : EsfValueNode<List<EsfNode>>, INamedNode {
        ...
        public override int GetHashCode() {
            // AllNodes builds a fresh list per call, so hashing it gives a different value every
            // time and does not agree with Equals. The name alone is stable.
...
//                result += " " + val.ToXml();
        ...
        public virtual void AddNode(EsfNode childNode) {
            if (Codec is AbcfFileCodec && childNode is StringNode) {
                var codec = Codec as AbcfFileCodec;
                var value = (childNode as StringNode).Value;

                if (childNode is AsciiStringNode)       codec.AddAsciiName(value);
                else if (childNode is Utf16StringNode)  codec.AddUtf16Name(value);
            }
